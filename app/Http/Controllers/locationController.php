...
class locationController extends Controller
{
    ...
    $lokasi_toko = DB::table('lokasi_toko')->get();
		return view('location/location',['lokasi_toko'=>$lokasi_toko]);
    }
    
    public function titikawal(){
		return view('location/inputtitikawal');
    }

    public function titikkunjungan(){
		return view('location/titikkunjungan');
    }

    public function store (Request $request){
      $id=(DB::table('lokasi_toko')->count('barcode'))+1;
	    $barcode = "TK".str_pad($id,3,"0",STR_PAD_LEFT);
    ...
    public  function cetak(){ 
      ...
      $pdf =  PDF::loadView  ('location/cetakBarcode',  compact('lokasi', 'no')); 
      $pdf->setPaper("f4"); 
      return $pdf->stream(); 
    }
    ...
    public  function cetakkk(){ 
      ...
      $paper_width = 793.7007874; // 38 mm
          $paper_height = 623.62204724; // 18 mm
          $paper_size = array(0, 0, $paper_width, $paper_height);
      $pdf =  PDF::loadView  ('location/cetak',  compact('lokasi', 'no')); 
      $pdf->setPaper("f4"); 
      return $pdf->stream(); 
    }
    ...
    public  function cetakBarcodeId($id){ 
      ...
      $paper_width = 793.7007874; // 38 mm
          $paper_height = 623.62204724; // 18 mm
          $paper_size = array(0, 0, $paper_width, $paper_height);
      $pdf =  PDF::loadView  ('location/cetakBarcodeId',  compact('lokasi', 'no')); 
      $pdf->setPaper('letter'); 
      return $pdf->stream(); 
    }
}